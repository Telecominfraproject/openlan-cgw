...
impl ProxyConnectionServer {
    ...
    async fn process(self: Arc<Self>, mut rx_mbox: ProxyConnectionServerMboxRx) {
        ...
        loop {
            ...
            if let Ok(true) = self.proxy_remote_discovery.check_redis_updated(&mut self.last_sync_timestamp).await {
                should_resync = true;
    ...
    async fn set_peer_connection(&self, 
        ...
        let peer_msg = ProxyConnectionProcessorReqMsg::SetPeer(mac);
