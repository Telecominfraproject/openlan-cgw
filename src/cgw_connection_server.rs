...
use crate::cgw_nb_api_listener::{
    ...
    cgw_construct_infra_group_topomap_generate_response, cgw_construct_infra_join_msg,
...
enum CGWNBApiParsedMsgType {
    ...
    InfrastructureGroupTopomapGenerate(Option<ConsumerMetadata>),
...
impl CGWNBApiParsedMsg {
    ...
    async fn handle(self, server: Arc<CGWConnectionServer>) {
        ...
        match self.msg_type.clone() {
            ...
            CGWNBApiParsedMsgType::InfrastructureGroupTopomapGenerate(consumer_metadata) => {
                self.handle_infrastructure_group_topomap_generate(
                    server,
                    consumer_metadata,
                    timestamp,
                )
    ...
    async fn handle_infrastructure_group_topomap_generate(
        &self,
        server: Arc<CGWConnectionServer>,
        consumer_metadata: Option<ConsumerMetadata>,
        timestamp: i64,
        ...
        let reporter_shard_id = server.get_local_id();
        let response: Result<String>;
        ...
        let cloud_header: Option<String> = cgw_construct_cloud_header(group_cloud_header, None);

        if !server.feature_topomap_enabled {
            response = cgw_construct_infra_group_topomap_generate_response(
                gid,
                reporter_shard_id,
                0,
                uuid,
                ...
                false,
                Some(format!(
                    "Topology Map feautre is disabled for CGW_ID{reporter_shard_id}"
                )),
                consumer_metadata,
                timestamp,
            );
        ...
        if let Ok(resp) = response {
            ...
            error!("Failed to construct infrastructure_group_topomap_generate message!");
        }
    }
}
...
impl CGWConnectionServer {
    ...
    fn parse_nbapi_msg(&self, payload: &str) -> Result<CGWNBApiParsedMsg> {
        ...
        #[derive(Debug, Serialize, Deserialize)]
        struct InfraGroupTopomapGenerate {
            r#type: String,
            infra_group_id: i32,
            uuid: Uuid,
            consumer_metadata: Option<ConsumerMetadata>,
        ...
        match msg_type {
            ...
            "infrastructure_group_topomap_generate" => {
                let json_msg: InfraGroupTopomapGenerate = match serde_json::from_str(payload) {
                    Ok(json) => json,
                    Err(e) => {
                        return Err(Error::ConnectionServer(format!("Failed to parse infrastructure_group_topomap_generate message! Error: {e}")));
                    }
                };
                return Ok(CGWNBApiParsedMsg::new(
                    json_msg.uuid,
