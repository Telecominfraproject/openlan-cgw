...
class Device:
    ...
    def send_partial_state(self, ss_socket: client.ClientConnection):
        str_frame = Frame(OP_TEXT, bytearray(self.messages.state, 'utf-8')).serialize(mask=False)
        if self.sent_part is False:
            print("Sent first slice {}", str_frame[:40])
            ss_socket.socket.send(str_frame[:40])
            self.sent_part = True
            return
        ...
        try:
            ...
            while True:
                ...
                if time.time() - start > self.interval:
                    ...
                    start = time.time()
                    self.send_partial_state(self._socket)
    ...
    def job(self):
        ...
        if self.stop_event.is_set():
            return
        logger.debug("starting simulation {}", self.mac)
        self.connect()
        start = time.time()
        try:
            self.send_hello(self._socket)
